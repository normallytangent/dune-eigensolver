...
def main():
    ...
    # Testing correctness
    W, E = arpack_backend(Asneu, Bsneu, m, shift, tol, maxiter)
    A = blas.daxpy(Bsneu, Asneu, a=shift)
    # is_match = np.zeros((m),dtype=bool)
    # for i in range(0,m):
    ...
    # print(is_match)
    # print(all(is_match))
    print(np.allclose(A @ Eveq_neu - Bsneu @ Eveq_neu @ (np.diag(Eval_neu)+shift * np.eye(m)), np.zeros((N**2, m))))
